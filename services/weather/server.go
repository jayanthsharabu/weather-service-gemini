...
type openMeteoResponse struct {
	Current struct {
		...
		WindDirection int32   `json:"wind_direction_10m"`
...
func (s *weatherService) GetCurrentWeather(ctx context.Context, req *weatherpb.WeatherRequest) (*weatherpb.WeatherResponse, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("API Requests failed: %v", err)
	}
	...
	response := &weatherpb.WeatherResponse{
		...
		Temperature: fmt.Sprintf("%.1f°C", weatherData.Current.Temperature),
		FeelsLike:   fmt.Sprintf("%.1f°C", weatherData.Current.Temperature),
		TempMin:     fmt.Sprintf("%.1f°C", weatherData.Current.Temperature),
		TempMax:     fmt.Sprintf("%.1f°C", weatherData.Current.Temperature),
		Pressure:    1013,
		Humidity:    weatherData.Current.Humidity,
		WindSpeed:   weatherData.Current.WindSpeed,
		WindDeg:     weatherData.Current.WindDirection,
